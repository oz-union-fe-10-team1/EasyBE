...
from .models import TasteAnswer, TasteType, TasteTypeRecommendation, UserProfile
...
class TasteScoreCalculator:
    ...
    @classmethod
    def _check_gourmet_type(cls, basic_scores: Dict[str, int]) -> Optional[Dict[str, Any]]:
...
class TasteAnalyzer:
    ...
    def _get_recommended_products(self, taste_type: str) -> List[Dict]:
        ...
        try:
            ...
            # 맛 타입별 추천 상품 조회
            recommendations = (
                TasteTypeRecommendation.objects.filter(taste_type__type_name=taste_type)
                .select_related("taste_type")
                .order_by("recommendation_order")[:3]
            )
