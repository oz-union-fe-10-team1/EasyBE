...
class TasteAnswer(models.Model):
    ...
    score_vector = models.JSONField(
        default=dict,
        help_text="맛 계수에 미치는 영향 (JSON 형태) - 예: {'달콤과일': 1}"
    )
...
class TasteType(models.Model):
    ...
    type_name = models.CharField(
        max_length=100,
        help_text="맛 타입명 (달콤과일, 상큼톡톡, 목적여운, 깔끔고소, 미식가)"
    )
    ...
    type_image_url = models.URLField(
        blank=True,
        help_text="타입 이미지 URL"
    )
...
class UserProfile(models.Model):
    ...
    taste_test_completed_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text="테스트 완료 시간"
    )
