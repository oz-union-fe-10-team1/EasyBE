...
from .models import TasteTest, TasteQuestion, TasteAnswer, TasteType, UserProfile
...
class TasteTestSubmitSerializer(serializers.Serializer):
        ...
        for i, answer in enumerate(value):
            ...
            if missing_fields:
                raise serializers.ValidationError(
                    f"답변 {i + 1}에 필수 필드가 누락되었습니다: {missing_fields}"
                )
    ...
    def _validate_questions_and_answers(self, answers):
        ...
        for answer in answers:
            ...
            try:
                ...
                raise serializers.ValidationError(
                    f"질문 ID {question_id}가 이 테스트에 존재하지 않습니다."
                )
            except TasteAnswer.DoesNotExist:
                raise serializers.ValidationError(
                    f"답변 ID {answer_id}가 질문 ID {question_id}에 존재하지 않습니다."
                )
    ...
    def create(self, validated_data):
        ...
        try:
            ...
            logger.info(f"맛 테스트 완료 - User: {request.user.id}, Type: {result.profile.initial_taste_type.type_name}")
            return result
...
class UserProfileSerializer(serializers.ModelSerializer):
    """사용자 프로필 시리얼라이저"""
    taste_type_id = serializers.IntegerField(
        source='initial_taste_type.id',
        read_only=True,
        allow_null=True
    )
    taste_type_name = serializers.CharField(
        source='initial_taste_type.type_name',
        read_only=True,
        allow_null=True
    )
    taste_type_description = serializers.CharField(
        source='initial_taste_type.type_description',
        read_only=True,
        allow_null=True
    )
    taste_type_image_url = serializers.URLField(
        source='initial_taste_type.type_image_url',
        read_only=True,
        allow_null=True
    )
    ...
    class Meta:
        model = UserProfile
        fields = [
            'id', 'has_completed_test', 'taste_test_completed_at',
            'taste_type_id', 'taste_type_name', 'taste_type_description', 'taste_type_image_url',
            'type_count', 'all_scores', 'characteristics',
            'algorithm_status', 'created_at', 'updated_at'
        ]
