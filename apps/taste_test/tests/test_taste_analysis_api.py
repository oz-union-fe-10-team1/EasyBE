...
from apps.taste_test.models import TasteTest, TasteQuestion, TasteAnswer, TasteType
...
class TasteTestLogicTests(TestCase):
    ...
    def setup_real_questions(self):
        ...
        # Q3: 디저트를 딱 하나만 고른다면?
        self.q3 = TasteQuestion.objects.create(
            test=self.test,
        ...
        # Q4: 평소 즐겨 마시는 음료는?
        self.q4 = TasteQuestion.objects.create(
            test=self.test,
    ...
    def test_모든_0점_기본값(self):
        ...
        empty_answer = TasteAnswer.objects.create(
            question=self.q1,
            answer_text="테스트용 빈 답변",
            score_vector={}
        )
    ...
    def test_실제_플로우차트_시나리오들(self):
        ...
        scenarios = [
            {
                ...
                'answers': [self.q1_a.id, self.q2_b.id, self.q3_b.id,
                            self.q4_b.id, self.q5_b.id, self.q6_a.id],
                'expected_winner': '달콤과일'
            },
            {
                'name': '상큼톡톡 청량파',
                'path': 'B-A-A-A-A-B',
                'answers': [self.q1_b.id, self.q2_a.id, self.q3_a.id,
                            self.q4_a.id, self.q5_a.id, self.q6_b.id],
                'expected_winner': '상큼톡톡'
            },
            {
                'name': '목적여운 탐험가',
                'path': 'B-A-B-B-B-B',
                'answers': [self.q1_b.id, self.q2_a.id, self.q3_b.id,
                            self.q4_b.id, self.q5_b.id, self.q6_b.id],
        ...
        for scenario in scenarios:
            with self.subTest(scenario=scenario['name']):
                answers = [
                    {'question_id': i + 1, 'answer_id': answer_id}
                    for i, answer_id in enumerate(scenario['answers'])
                ]
                ...
                if result['primary_type'] != '미식가' and '×' not in result['primary_type']:
                    max_score_type = max(
                        [k for k in scores.keys() if k != '미식가'],
